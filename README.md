...
匹配成功时 `quality` 为实际取到的音质，并在 SPlayer 日志中记录改用的平台。两个平台都没有匹配时抛出 `CHKSZ_TRACK_UNAVAILABLE`，错误信息包含歌名和网易云的原始错误。每次跨平台匹配最多额外消耗 4 次 ChKSz 额度（两次搜索 + 两次解析），不希望消耗额度时可在插件配置中关闭该开关。
