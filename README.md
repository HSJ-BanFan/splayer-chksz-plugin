...
> 建议同时在 **设置 → 播放器 → 在线音源路由** 里了解「插件优先」模式：默认是官方接口先解析，官方拿不到地址时才轮到本插件。详见 [何时会走插件](#何时会走插件)。
...
> 跨平台能否成功取决于 ChKSz 的 QQ 音乐与酷狗通道是否可用：实测出现过 QQ 音乐搜索接口对**任何关键词**（含官方文档示例）都返回 `404 未找到匹配的歌曲`、酷狗持续返回 `502/503` 的情况。此时插件不会把服务端故障伪装成"没搜到这首歌"，而是抛出 `CHKSZ_CROSS_PLATFORM_UNAVAILABLE` 并列出各平台的真实状态。
...
- 某个平台返回 `502`/`503`/`504` 视为该平台上游故障，进入 2 分钟通道冷却；冷却期内跨平台匹配跳过该平台。
...
**为什么有些歌还是放不出来？**
三类原因：① 网易云没版权，而 ChKSz 的 QQ 音乐/酷狗通道当前不可用（插件会明确报 `CHKSZ_CROSS_PLATFORM_UNAVAILABLE`，等服务端恢复即可）；② 额度受限（返回 `CHKSZ_HTTP_429`，等一分钟再试）；③ 地址本身可拉流、但客户端解码失败——插件拿到的是完全正常的响应，这种情况插件日志不会报错。
...
插件优先会在官方接口之前发起 ChKSz 请求，可能增加配额消耗；若插件没有返回可用地址，客户端仍会回退到官方接口。
...
| `CHKSZ_HTTP_429` | 超过速率限制，并显示 `Retry-After` |
...
- 跨平台依赖 ChKSz 的 QQ 音乐与酷狗通道，这两个通道可能整体不可用（返回 404 或 502/503）。此时无版权歌曲只能等服务端恢复。
