...
| `kg` | `/api/kugou_music` | 酷狗音乐（按 `id/hash` 解析） |
...
3. 按歌名、歌手和时长校验候选，避免把 Live、Remix 或其他版本当成原曲；QQ/酷狗必须有可校验的详情时长，网易云搜索接口不保证时长，版本宽松匹配在缺少时长时会被拒绝；
...
- 某个平台返回 `502`/`503`/`504` 视为该平台上游故障，进入通道冷却：有 `Retry-After` 时按它（最长 15 分钟），否则 2 分钟；冷却期内无论它是主渠道还是跨平台目标都会跳过，避免重复消耗请求。
...
| `CHKSZ_CHANNEL_COOLDOWN` | 某个平台处于上游故障冷却期，未发出该平台请求 |
